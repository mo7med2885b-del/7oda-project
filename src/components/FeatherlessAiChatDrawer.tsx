...
export const FeatherlessAiChatDrawer: React.FC<FeatherlessAiChatDrawerProps> = ({ isOpen, onClose, initialPrompt }) => {
  ...
  const parseBold = (text: string) => {
    ...
    return parts.map((part, i) => {
      if (part.startsWith('**') && part.endsWith('**')) {
        return <strong key={i} className="font-black text-[#6AB8FF]">{part.slice(2, -2)}</strong>;
      }
      return part;
    });
  };
    ...
    while (i < rawLines.length) {
      ...
      if (line.trim().startsWith('|') && line.trim().endsWith('|')) {
        ...
        if (tableLines.length >= 2) {
          ...
          elements.push(
            <div key={`table-${i}`} className="my-2.5 overflow-x-auto rounded-xl border border-[#6AB8FF]/30 bg-[#22262B] shadow-md">
              <table className="w-full text-left rtl:text-right border-collapse text-[11px]">
                <thead>
                  <tr className="bg-[#2C3137] text-[#6AB8FF] border-b border-[#6AB8FF]/30 font-bold uppercase tracking-wider">
                    {headerCells.map((h, hIdx) => (
                      <th key={hIdx} className="py-2 px-3">{parseBold(h)}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#6AB8FF]/20">
                  {dataRows.map((r, rIdx) => (
                    <tr key={rIdx} className="hover:bg-[#2C3137]/60 transition">
                      {r.map((cell, cIdx) => (
      ...
      // Headers
      if (line.startsWith('### ')) {
        elements.push(<h4 key={i} className="font-bold text-xs text-[#6AB8FF] mt-2 mb-1">{parseBold(line.replace('### ', ''))}</h4>);
      } else if (line.startsWith('## ')) {
        elements.push(<h3 key={i} className="font-extrabold text-xs text-[#6AB8FF] mt-2.5 mb-1">{parseBold(line.replace('## ', ''))}</h3>);
      } else if (line.startsWith('# ')) {
        elements.push(<h2 key={i} className="font-black text-sm text-[#6AB8FF] mt-3 mb-1.5">{parseBold(line.replace('# ', ''))}</h2>);
        ...
        elements.push(
          <div key={i} className="flex items-start gap-1.5 my-1 pr-2 rtl:pr-2 rtl:pl-0">
            <span className="text-[#6AB8FF] font-bold shrink-0">•</span>
            <span className="leading-relaxed">{parseBold(listContent)}</span>
          </div>
        );
      } else if (line.trim() === '---') {
        elements.push(<hr key={i} className="my-2.5 border-[#6AB8FF]/30" />);
      } else if (!line.trim()) {
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-50 w-full max-w-md bg-[#2C3137] border-l border-[#6AB8FF]/30 shadow-2xl flex flex-col text-white transition-all transform duration-300">
      {/* Clean Drawer Header */}
      <div className="p-4 bg-[#22262B] border-b border-white/10 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <div className="w-9 h-9 rounded-xl bg-[#6AB8FF] text-slate-950 flex items-center justify-center font-bold shadow">
            <Sparkles className="w-5 h-5" />
          </div>
          <div>
            <h3 className="font-bold text-sm text-[#6AB8FF]">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 text-xs">
        {messages.map((msg, index) => (
          ...
          >
            {msg.role === 'assistant' && (
              <div className="w-7 h-7 rounded-lg bg-[#2C3137] text-[#6AB8FF] flex items-center justify-center shrink-0 mt-0.5">
            ...
            <div
              className={`p-3.5 rounded-2xl max-w-[90%] ${
                msg.role === 'user'
                  ? 'bg-[#2C3137] text-white font-medium rounded-tr-none'
                  : 'bg-[#22262B] border border-[#6AB8FF]/20 text-slate-100 rounded-tl-none shadow'
              }`}
            >
            ...
            {msg.role === 'user' && (
              <div className="w-7 h-7 rounded-lg bg-[#6AB8FF] text-slate-950 flex items-center justify-center shrink-0 mt-0.5">
        ...
        {loading && (
          <div className="flex items-center gap-2 text-xs text-[#6AB8FF] p-2.5 bg-[#22262B] rounded-xl border border-[#6AB8FF]/30 w-fit shadow">
      ...
      {/* Input Bar */}
      <form onSubmit={handleSendMessage} className="p-3 bg-[#22262B] border-t border-white/10 flex items-center gap-2">
        <input
          type="text"
          value={inputMessage}
          onChange={e => setInputMessage(e.target.value)}
          placeholder={lang === 'ar' ? 'اكتب سؤالك (مثال: تحليل الملف أو المواعيد)...' : 'Type your query here...'}
          className="flex-1 p-2.5 rounded-xl bg-[#2C3137] border border-[#6AB8FF]/30 text-white placeholder-slate-400 text-xs focus:outline-none focus:border-[#6AB8FF]"
        />
        <button
          type="submit"
          disabled={loading || !inputMessage.trim()}
          className="p-2.5 rounded-xl bg-[#6AB8FF] hover:bg-[#4FA5F5] text-slate-950 font-black disabled:opacity-50 transition shadow"
        >
