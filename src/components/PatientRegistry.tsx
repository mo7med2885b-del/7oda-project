...
export const PatientRegistry: React.FC<PatientRegistryProps> = ({ onViewPatientDossier, onOpenSoapEditor, onOpenAiPrompt }) => {
  ...
  const handlePatientAiAnalysis = (p: typeof patients[0]) => {
    if (!onOpenAiPrompt) return;
    const prompt = `قم بإجراء تحليل كلينيكي شامل لملاحظات المريضة:\n• الاسم: ${p.full_name}\n• السن: ${p.age} سنة (${p.gender})\n• فصيلة الدم: ${p.blood_type}\n• التنبيهات: ${p.medical_alerts || 'لا توجد'}\n• الحساسية: ${p.allergies || 'لا توجد'}\nما هي أفضل خطة علاج ومتابعة مقترحة لهذه الحالة؟`;
